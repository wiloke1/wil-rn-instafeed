...
const getHtmlVideo = (videoUrl, imageSrc, width, height) => `
  ...
  `;
const ImageCardFeatured = ({ imageSrc, height, width, isVideo, videoUrl, gallery }) => {
    ...
    const renderVideo = (videoUrl, imageSrc, isActive = true) => {
        return (<View style={styles.webviewWrap}>
        {!videosLoaded[videoUrl] && (<View style={styles.imageUnderlay}>
            {renderImage(imageSrc)}
            <View style={styles.videoLoading}>
              <ActivityIndicator size="large" color="#fff"/>
            </View>
          </View>)}
        {isActive ? (<WebView ref={videoRef} source={{ html: getHtmlVideo(videoUrl, imageSrc, width, height) }} onLoadEnd={() => {
        ...
        }} scrollEnabled={false} style={{ width, height }} onMessage={() => {
            StatusBar.setHidden(true);
        }}/>) : (<View style={{ width, height }}/>)}
      </View>);
    };
    if (!!gallery) {
        return (<Carousel horizontal pagingEnabled removeClippedSubviews showsHorizontalScrollIndicator={false} nestedScrollEnabled data={gallery} keyExtractor={item => item.shortcode} renderItem={({ item }, isActive) => {
            if (item.video) {
