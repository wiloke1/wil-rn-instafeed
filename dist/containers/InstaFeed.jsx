...
const InstaFeed = ({ settings = __wilInstagramShopify__, slotId, containerWidth, useNavigation = false, navigation, screenName, }) => {
    ...
    const handleOpenModal = (id, link, index) => () => {
        if (useNavigation) {
    ...
    const renderInstaItem = (item, index) => {
        return (<TouchableOpacity activeOpacity={1} style={styles.imageWrap} onPress={handleOpenModal(item.shortcode, item.link, index)}>
    ...
    const renderContent = () => {
        switch (setting.template) {
            case 'grid':
            case 'grid-brick':
                return (<View style={{ width: containerWidth }}>
            <Grid gap={gap} column={column} data={getInstaGridData(instaSection, rowState, column)} keyExtractor={item => item.shortcode} renderItem={renderInstaItem}/>
            {rowState * column < (instaSection.posts?.length || Infinity) && (<View style={styles.loadmore}>
            ...
            case 'slider':
                return (<View style={{ width: containerWidth, overflow: 'hidden' }}>
            <FlatList horizontal removeClippedSubviews showsHorizontalScrollIndicator={false} data={splitCarouselData(instaSection.posts || [], column, rowState)} keyExtractor={item => JSON.stringify(item)} renderItem={({ item }) => {
                    return (<View style={{ width: containerWidth - 10, paddingHorizontal: gap / 2 }}>
                    <Grid gap={gap} column={column} data={item} keyExtractor={item => item.shortcode} renderItem={item => {
